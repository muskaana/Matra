              ...
              {levels.map((level, index) => {
                const content = (
                  <div className={`flex items-center gap-5 rounded-lg p-2 -m-2 transition-colors ${level.locked ? 'cursor-not-allowed opacity-50' : 'cursor-pointer hover:bg-gray-50'}`} data-testid={`button-level-${level.id}`}>
                    <div className={`w-[80px] h-[80px] rounded-full flex items-center justify-center text-white font-bold text-3xl flex-shrink-0 border-[3px] border-white shadow-md transition-colors ${level.locked ? 'bg-gray-400' : 'bg-[#ff9930] hover:bg-[#CF7B24]'}`}>
